...
基于 Tauri 2 构建的桌面 AI 工作台，把主聊天窗口、助手管理、本地知识库、悬浮形态和桌面宠物整合在同一套应用里。

当前维护的是正在持续演进的 Tauri 版本，不再是 Electron 迁移分支，也不是只有基础壳层的最小原型。

## 项目定位

更像一个桌面 AI 工作台，而不是单一聊天窗口。关注的是：
...
作为 Tauri 桌面应用，开发与启动桌面窗口使用以下命令：

```bash
pnpm tauri dev
```

该命令会同时编译 Rust 后端和前端，弹出桌面窗口（主窗口、紧凑/宠物窗口都在里面），并支持代码热重载。这是运行应用的**主要命令**。

> 仅运行 `pnpm dev` 只会启动 Vite 网页，没有桌面窗口、没有 Tauri 运行时、也没有本地 SQLite 数据库，一般只用于纯前端调试；正常使用直接用 `pnpm tauri dev`。

补充说明：

- Vite 开发端口固定为 `1420`
- `tauri dev` 会复用这个端口作为前端入口
...
知识库更适合做本地文档整理和检索增强问答，上传与处理的边界如下：

- 文本、Markdown、代码、`pdf`、`docx` 支持基础导入与解析
- 图片和音频文件需要在对应知识库开启并配置多模态模型后再上传
- 当前版本不支持视频上传到知识库

想了解内部的 RAG 流程，可以继续阅读 [docs/rag-flow.md](docs/rag-flow.md)。
