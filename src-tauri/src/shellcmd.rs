...
/// 与工具结果通用 spill 管道的默认上限保持一致（见 `tool_output_spill`）。
pub(crate) const MAX_OUTPUT_CHARS: usize = crate::tool_output_spill::DEFAULT_TOOL_RESULT_CHARS;
...
///
/// 实际实现在 `tool_output_spill` 模块（shell / git / MCP 共用一套 head/tail 预览 + 落盘），
/// 这里只按 `bash` 工具名委托过去。
///
...
pub(crate) fn cap_output(text: &str, max: usize) -> String {
    crate::tool_output_spill::cap_and_spill(text, max, "bash")
}
...
mod tests {
    ...
    fn cap_output_leaves_short_text_untouched() {
        assert_eq!(cap_output("hello", 10), "hello");
    }
}
