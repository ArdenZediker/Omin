Create a single horizontal sprite strip for the Codex app digital pet `omni-schnauzer` in the state `jumping`.
...
Output exactly 5 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: a cute mini schnauzer puppy desktop pet for Omni, charcoal and silver fur, big eyes, blue collar with gold tag, compact chibi body, codex digital pet style.
